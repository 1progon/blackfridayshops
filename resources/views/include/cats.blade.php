...
<div class="my-2">
    <div class="d-flex list-group rounded-0" style="max-height: 100vh; overflow-y: scroll">
        @forelse($cats as $cat)
            <a class="list-group-item list-group-item-dark list-group-item-action font-weight-bold {{url()->current()
             == route
            ('category', $cat->slug)
            ?'active': ''}}"
            ...
            <div class="d-flex flex-wrap rounded-0">
                @forelse($cat->subCategories as $subCat)
                    <a class="list-group-item list-group-item-dark list-group-item-action py-0
